    ...
    <v-navigation-drawer permanent clipped app>
      <v-expansion-panel>
        <v-expansion-panel-content>
          <div slot="header"><v-icon>account_balance_wallet</v-icon>    <b>Wallet</b></div>
          <v-list class="grey lighten-3">
            <template v-for="(item, i) in wallet_menu">
      ...
      <v-expansion-panel v-show="!notShow['contract']">
        <v-expansion-panel-content>
          <div slot="header"><v-icon>code</v-icon>    <b>Smart Contracts</b></div>
          <v-list class="grey lighten-3">
            <template v-for="(item, i) in contracts_menu">
              <v-divider dark v-if="item.divider" class="my-4" :key="i" v-show="!notShow[item.name]"></v-divider>
              <v-list-tile :key="i" v-else @click="changeView(item.name)" active-class="white" v-model="isCurrent[item.name]" v-show="!notShow[item.name]">
                <v-list-tile-action>
                  <v-icon>{{ item.icon }}</v-icon>
      ...
      <v-expansion-panel v-show="!notShow['apps']">
        <v-expansion-panel-content>
          <div slot="header"><v-icon>code</v-icon><v-spacer></v-spacer><b>Apps</b></div>
          <v-list class="grey lighten-3">
            <template v-for="(item, i) in apps_menu">
              <v-divider dark v-if="item.divider" class="my-4" :key="i" v-show="!notShow[item.name]"></v-divider>
              <v-list-tile :key="i" v-else @click="changeView(item.name)" active-class="white" v-model="isCurrent[item.name]" v-show="!notShow[item.name]">
                <v-list-tile-action>
                  <v-icon>{{ item.icon }}</v-icon>
      ...
      <v-list class="grey lighten-3">
        <template v-for="(item, i) in settings_menu">
          <v-divider dark v-if="item.divider" class="my-4" :key="i" v-show="!notShow[item.name]"></v-divider>
          <v-list-tile :key="i" v-else @click="changeView(item.name)" active-class="white" v-model="isCurrent[item.name]" v-show="!notShow[item.name]">
            <v-list-tile-action>
              <v-icon>{{ item.icon }}</v-icon>
  ...
  computed: {
    ...
    notShow() {
      return {
        ...
        apps: this.mode === 'offline' || !this.wallet,
      }
    },
    headerClass() {
